...
/**
 *
 * Ant样式路径模式的PathMatcher实现。 下面提供了示例。
 * 此映射代码的一部分已从Apache Ant 借来了。
 * 映射使用以下规则匹配URL：
 * ？ 匹配一个字符
 * *匹配零个或多个字符
 * **匹配路径中的零个或多个“目录”
 * 一些例子：
 * com/t?st.jsp匹配com/test.jsp但也匹配com/tast.jsp或com/txst.jsp
 * com/*.jsp匹配com目录中的所有.jsp文件
 * com/**\/test.jsp匹配com路径下的所有test.jsp文件
        *org/apache/shiro/**\/*.jsp匹配org/apache/shiro路径下的所有.jsp文件
        *org/**\/servlet/bla.jsp匹配org/apache/shiro/servlet/bla.jsp但也匹配org/apache/shiro/servlet/bla.jsp org/apache/shiro/testing/servlet/bla.jsp和org/servlet/bla.jsp
        *注意：该类是从Spring框架（ 经过修改）中借用的（非常感谢）。 我们不想重塑他们已经完成的伟大工作的车轮，但也不想强迫每个Shiro用户依靠Spring
 *
