...
public abstract class AuthenticatingRealm extends CachingRealm implements Initializable {
    ...
    /**
     *
     * 返回任何可用的Cache实例以用于身份验证缓存。 其功能如下：
     * 如果已经明确配置了authenticationCache （它不为null），则将其返回。
     * 如果未配置authenticationCache ：
     * 如果enabled身份验证缓存， cacheManager任何可用的cacheManager以获得可用的身份验证缓存。
     * 如果禁用了身份验证缓存，则此实现不执行任何操作
     *
