...
/**
 *
 * authc  认证  cation 是 账户信息
 *  authz  身份验证
 *
 * 仅表示与身份验证登录过程相关的主题（即用户）存储的帐户信息。
 *
 *
 * 这里存放的是 根据 后台数据库查询出来的数据
 *
 *
...
public interface AuthenticationInfo extends Serializable {
     ...
     * 这里指的是 数据库查询的数据信息
     *
     *  返回与相应主题关联的所有主体。每个主体都是对应用程序有用的标识信息，
     *  例如用户名或用户ID，给定名称等-对应用程序有用以标识当前<code> Subject <code>的任何信息。
     *    ID
     *    Account
     *    Phone
     *    唯一 Name
