...
/**
 *
 * SecurityManager界面的非常基本的起点，它仅提供日志记录和缓存支持。 所有实际的SecurityManager方法实现都留给子类。
 *
 * 1.0中的更改-实例化期间默认情况下未创建默认的CacheManager实例。 由于缓存策略可能会根据应用程序的需求而变化很大，
 * 因此，如果要启用跨框架的缓存，则必须显式配置CacheManager实例
 *
