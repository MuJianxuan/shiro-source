 ...
 *
 *
 Shiro支持将所有session操作委派给包装的SessionManager实例的SecurityManager类层次结构。
 也就是说，此类在SessionManager接口中实现了这些方法，但实际上，这些方法仅仅是对底层“真实” SessionManager实例的直通调用。

 此类未或其父级实现的其余SecurityManager方法将由子类实现。
