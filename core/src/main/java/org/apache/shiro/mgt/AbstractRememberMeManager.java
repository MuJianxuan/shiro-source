 ...
 * 由于AES是对称密码，因此使用相同的key来加密和解密数据，但是请注意：
 *
 * 因为Shiro是一个开源项目，所以如果有人知道您正在使用Shiro的默认key ，
 * 他们可以下载/查看源代码，并花费足够的精力来重建key并随意解码加密的数据。
 *
 * 当然，此密钥仅真正用于加密记住的PrincipalCollection ，通常是用户ID或用户名。
 * 因此，如果您不考虑敏感信息，并且认为默认密钥仍然使事情“非常困难”，那么您可以忽略此问题。
 *
 * 但是，如果您确实认为这构成敏感信息，建议您通过setCipherKey方法将自己的key提供给仅应用程序已知的密钥，
 * 以确保没有第三方可以解密您的数据。 您可以通过调用CipherService的generateNewKey方法并将该结果用作cipherKey配置属性来生成自己的密钥
 *
 *
 * @since 0.9
 */
public abstract class AbstractRememberMeManager implements RememberMeManager {
    ...
    /**
     * 只做了一些基础的数据存储 的getter setter 的处理和 密码服务的初始化
     */
    ...
    /**
     *  用于将PrincipalCollection实例转换为字节数组的序列化器
    ...
    /**
     *
     *  默认构造函数，将{@link DefaultSerializer}初始化为{@link getSerializer（）序列化器}，
     *  并将{@link AesCipherService}初始化为{@link getCipherService（）cipherService}。
     *
