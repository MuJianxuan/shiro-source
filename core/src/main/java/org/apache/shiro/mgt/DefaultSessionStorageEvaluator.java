...
/**
 *
 * 一个默认的SessionStorageEvaluator ，它提供对是否以及如何将Session用于存储Subject状态的合理控制。
 * 有关确切行为，请参见isSessionStorageEnabled(Subject)方法
 *
...
public class DefaultSessionStorageEvaluator implements SessionStorageEvaluator {
    ...
    /**
     * 确定主题会话是否可以用于保留主题状态（如果主题的会话尚不存在）的全局策略
     ...
     *
     此实现的功能如下：
     如果指定的Subject已经有一个Session （通常是因为应用程序开发人员已经调用了subject.getSession() ），那么Shiro将使用该已有Session来存储Subject的状态。
     如果某个主题还没有会话，则此实现将检查sessionStorageEnabled属性：
     如果sessionStorageEnabled为true（默认设置），则如有必要，可以创建一个新会话以保留主题状态。
     如果sessionStorageEnabled为false ，将不会创建新会话来保持会话状态。
     *
