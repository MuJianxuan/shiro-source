...
/**
 * 主题对象
 * 这些操作包括身份验证（登录注销），授权（访问控制）和会话访问。它是Shiro的单用户安全功能的主要机制。
 *
...
public interface Subject {
    ...
    /**
     *  返回此主题在应用程序范围内的唯一标识主体，如果该主题是匿名的，则返回{@code null}，因为它尚无任何关联的帐户数据
     *    匿名： null
     *    术语<em> principal <em>只是一个花哨的安全术语，
     *    用于表示应用程序用户的任何标识属性，
     *    例如用户名，用户ID或公钥，或您可能在应用程序中使用的其他任何形式。识别用户。
     *
    ...
    /**
     *
     *   例如，用户ID，姓（姓），给定（名字），社会保险号，昵称，用户名等都是委托人的示例。 账号ID 可用于登录使用
     *  以{@code PrincipalCollection}或{@code null}的形式返回此主题的主体（标识属性），如果该主题是匿名的，因为它尚无任何关联的帐户数据
     *
     ...
     * if they haven't logged in).  如果他们还没有登录
     * <p/>
     *
     *  单词“ principals”仅是一个花哨的安全术语，用于标识与“主题”（又名“应用程序”）用户相关联的属性。
     *  例如，用户ID，姓（姓），给定（名字），社会保险号，昵称，用户名等都是委托人的示例。
     *
    ...
    /**
     *  返回与此主题关联的应用程序Session 。 基于布尔参数，此方法的功能如下：
     * 如果已经存在与此Subject相关联的会话，则将其返回并忽略create参数。
     * 如果不存在任何会话且create为true ，则将创建一个新会话，并将其与此Subject关联，然后返回。
     * 如果不存在会话且create为false ，则返回null
     *
    ...
    //////=============== 上面都是有关 登录用户的一些方法   如 认证、登录、登出、


    /////////============   线程池 创建方法
    /**
     *   将指定的{@code Callable}与此{@code Subject}实例相关联，然后在当前运行的线程上执行它。
     *   如果要在其他线程上执行{@code Callable}，则最好改用{@link associateWith（Callable）}方法。
     *
