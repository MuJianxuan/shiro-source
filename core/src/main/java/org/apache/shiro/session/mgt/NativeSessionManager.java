...
public interface NativeSessionManager extends SessionManager {
    ...
    /**
     *
     *  如果关联的会话有效（该会话存在并且没有停止也没有过期），则返回true否则返回false
     *
    ...
    /**
     *
     *  如果关联的会话有效（它存在并且没有停止或过期），则InvalidSessionException ，或者抛出InvalidSessionException指示会话ID无效。
     *  最好使用它代替isValid因为抛出的任何异常都将明确解释无效的原因。
     *
