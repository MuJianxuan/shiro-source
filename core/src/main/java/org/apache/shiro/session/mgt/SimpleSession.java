...
public class SimpleSession implements ValidatingSession, Serializable {
    ...
    //
/**
 *     以下字段被标记为瞬态，以避免双重序列化。它们实际上是序列化的（即使“ transient”通常表示不是这样），
 *     但是它们是通过此类中的writeObject和readObject实现显式序列化的。如果我们没有将它们声明为瞬态的，
 *     则out.defaultWriteObject（）; writeObject中的call也会对所有非瞬态字段进行序列化，
 *     从而有效地对字段进行双倍序列化（也使序列化大小加倍）。讨论中涵盖了此发现：
 */
    ...
    /**
     *
     * 返回会话停止的时间；如果会话仍处于活动状态，则返回null 。
     *
     * 在多种情况下，会话可能会停止：
     * 如果用户注销系统，则其当前会话将终止（释放）。
     * 如果会话过期
     * 应用程序显式调用stop()
     * 如果发生内部系统错误，并且会话状态不能再准确反映用户的行为，例如在系统崩溃的情况下
     *
     * 一旦停止，则可能不再使用会话。 它被锁定，无法进行任何进一步的活动
     *
