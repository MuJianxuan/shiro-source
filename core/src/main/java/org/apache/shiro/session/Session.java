 ...
 *  session 对象
 *
 *  Session是与有一段时间与软件系统交互的单个Subject（用户，守护程序等）相关联的有状态数据上下文。
 *  Session旨在由业务层进行管理，并且可以通过其他层进行访问，而不受任何给定的客户端技术的束缚。
 *  这对Java系统是一个巨大的好处，因为直到现在，
 *  唯一可行的会话机制是javax.servlet.http.HttpSession或有状态会话EJB，它们多次不必要地将应用程序耦合到Web或ejb技术
 *
