...
/**
 *
 * ShiroHttpServletRequest包装了Servlet容器的原始ServletRequest实例，
 * 但确保需要Shiro的所有需要​​Shiro支持的HttpServletRequest调用
 * （ getRemoteUser ， getSession等）都可以先由Shiro执行，然后再允许调用底层Servlet容器实例的方法
 *
 *
...
public class ShiroHttpServletRequest extends HttpServletRequestWrapper {
    ...
    // Web中的默认的Session  Java定义了 HttpSession

    protected HttpSession session = null;
    protected boolean httpSessions = true;

    /**
     * 初始化的时候应该在 哪呢？ 在 filter 的初始化(init )中吧
     * @param wrapped
