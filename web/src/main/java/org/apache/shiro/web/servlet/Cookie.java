...
/**
 *
 * 表示HTTP cookie操作的接口，为所有属性（包括HttpOnly 支持）支持pojo样式的getter和setter。
 * 这使Shiro甚至可以在基于2.4和2.5 API的Servlet容器上设置HttpOnly  cookie（仅在2.6规范中引入了Servlet API'native'支持）
 *
 *
...
public interface Cookie {
    ...
    /**
     * Set-Cookie HTTP响应标头的SameSite属性允许您声明cookie是否应限于第一方或同一站点上下文。
