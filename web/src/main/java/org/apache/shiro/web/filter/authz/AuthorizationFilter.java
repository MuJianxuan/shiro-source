...
public abstract class AuthorizationFilter extends AccessControlFilter {
     ...
     * 当访问被拒绝时处理响应。 它的行为如下：
     * 如果Subject未知[1] ：
     * 传入的请求将被保存，并将被重定向到登录页面以进行身份验证（通过saveRequestAndRedirectToLogin(ServletRequest, ServletResponse)方法）。
     * 成功通过身份验证后，它们将被重定向回最初尝试的页面。
     * 如果主题是已知的：
     * 将设置HTTP HttpServletResponse.SC_UNAUTHORIZED标头（401未经授权）
     * 如果已配置了unauthorizedUrl URL，将向该URL发出重定向。 否则401响应会正常显示
     * [1] ： Subject是“已知”的subject. getPrincipal() subject. getPrincipal()不为null ，这意味着该主题当前已通过身份验证，或者已通过“记住我”服务被记住
     *
