...
/**
 * 后备List实例和简单name属性支持的简单NamedFilterList实现。 所有List方法的实现都立即委派给包装的后备列表
 *
...
public class SimpleNamedFilterList implements NamedFilterList {
    ...
    /**
     * 使用指定的name创建一个新的SimpleNamedFilterList实例，默认情况下使用一个新的ArrayList实例作为后备列表。
     *
    ...
    /**
     * 使用指定的name和backingList创建一个新的SimpleNamedFilterList实例。
