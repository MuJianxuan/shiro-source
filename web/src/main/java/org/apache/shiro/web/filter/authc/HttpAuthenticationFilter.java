...
/**
 * Http 认证 过滤器
 *   要求进行请求的用户进行authenticated以使请求继续进行；如果未通过，
 *   则要求用户通过HTTP“ Authentication”标头（例如BASIC，Bearer等）登录。被允许继续请求的资源/ URL。
 *
...
abstract class HttpAuthenticationFilter extends AuthenticatingFilter {
    ...
    /**
     * 设置在发送HTTP基本质询响应时此过滤器将使用的HTTP WWW-Authenticate标头方案。 默认值为BASIC 。
     *
    ...
    /**
     * 基本身份验证筛选器可以使用应应用的HTTP方法列表进行配置。 此方法确保仅对指定的那些HTTP方法才需要身份验证。 例如，如果您有配置：
     *           [urls]
     *           /basic/** = authcBasic[POST,PUT,DELETE]
     *
     * 那么GET请求将不需要身份验证，但需要POST
     *
     *
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 目的是: /basic/** = authcBasic[POST,PUT,DELETE]
        //  那么GET请求将不需要身份验证，但需要POST
    ...
    /**
     * 返回一个包含[Authorization]标头中解析的凭证部分的String [].
     *
